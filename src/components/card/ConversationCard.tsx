...
import { setConversationProfile, setConversationId, setProfileChange, setConversationType } from "../../redux/slices/Conversation";
...
const ConversationCard: React.FC<CardPropsType> = ({ data, socket }) => {
    ...
    const clickHandler = useCallback(() => {
        if (conversation.profile && (data.isGroup ? (conversation.profile._id !== data._id) : (conversation.profile._id !== data.user?._id))) {
            let dataId = conversation.profile._id;
            const isExist = conversations.data.find((ele) => (ele.user?._id === dataId));
            if (isExist) dataId = isExist._id;
            socket?.emit("leave-room", dataId);
