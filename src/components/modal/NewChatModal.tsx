...
const NewChatModal: React.FC<ModalPropsType> = ({ setOpenModal, sendRequest, setRight, socket }) => {
      ...
      if (!image) {
        ...
        if (response.status) {
          ...
          if (response2.status) {
            ...
            resetMemberHandler();
            if (response2.data) {
              notifyGroupMembers(response2.data._id);
              goToChat(response2.data);
            }
          } else {
  ...
  const notifyGroupMembers = useCallback((conversationId: string) => {
    socket?.emit("notify-group-members", conversationId.toString());
  }, [socket]);
          ...
          </div>
          {
            tab === "member" && (
              <div className='w-[95%] mx-auto text-xs'>
                <input type='text' placeholder='Search' value={searchStr} onKeyUp={enterHandler} onChange={changeHandler}
              ...
              <form className='space-y-10' onSubmit={CreateGroup}>
                      ...
                      groupMembers.map((member, index) => {
                        return (
                          <div key={index} className='relative bg-[white] px-2 py-1'>
                            ...
                            <p className='absolute top-[-5px] right-[-5px] cursor-pointer' onClick={() => removeMemberHandler(member)}>
                ...
                <div className='flex justify-center items-center'>
                  <button type='submit' className='text-center bg-black text-[white] text-sm px-3 py-2 rounded-lg' disabled={disable}>
                    {
                      disable ? <ImSpinner9 size={18} className='animate-spin' /> : "Create"
                    }
                  </button>
                </div>
              </form>
